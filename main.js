...
var pathSelection = document.querySelectorAll(".js-path-selector");
for (i = 0; i < pathSelection.length; i++) {
	pathSelection[i].addEventListener("click", servePath);
}

// var pathTrigger = document.querySelectorAll(".js-path-trigger");
// for (i = 0; i < pathTrigger.length; i++) {
// 	pathTrigger[i].addEventListener("click", guide);
// }

var pathAnswer = document.querySelectorAll(".js-path-answer");
for (i = 0; i < pathAnswer.length; i++) {
	pathAnswer[i].addEventListener("click", guide);
}
...
function servePath() {
	...
	if (this.id == 'primaryResidence') {
		...
		for (i = 0; primaryResidenceTabs.length; i++) {
			...
			if (primaryResidenceTabs[i] !== 0) {
				primaryResidenceTabs[i].style.display = "none";
			}
		}
	}
...
function getQuestion() {
	var question = this.parentElement.childNodes[1].innerHTML;
...
function guide() {
	...
	var tab = 
	'<div class="primary-residence-child">' + 
	'<p class="tab-question">'+ question +'</p>' +
	'<div class="btn-group js-path-trigger" data-toggle="buttons" style="text-align: center;">' +
		'<label id="pr1a" class="btn btn-primary js-path-answer"><input class="radio-input" type="radio" name="options">'+ answer1 +'</label>' +
		'<label id="pr1b" class="btn btn-primary js-path-answer"><input class="radio-input" type="radio" name="options">'+ answer2 +'</label>' +
	'</div></div>';
	console.log(tab);
